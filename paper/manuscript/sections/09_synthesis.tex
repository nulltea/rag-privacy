...
into three views: the trade-off geometry that locates each family
...
which scheme is deployable depends on the application. The geometry below makes that trade
explicit, and the recommendations turn it into per-use-case guidance.

\subsection{Design Space}
\label{ssec:design-space}

Read across the families, one shape recurs: each maximizes at most two of the three
deployment-readiness criteria and pays on the third, and the security basis axis of
\cref{sec:threat-axes} explains why the third differs so much. Cryptographic inference
