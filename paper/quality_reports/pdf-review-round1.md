...
rebuilds; comments preserved here). Status as of this session.

Legend: ✅ done · 🟡 partial · ⏳ pending (staged) · ℹ️ answered, no change.
...
| 9 | §2.1 | "vignette" standard term? | ⏳ | Rename "vignette→case study" folded into the vignette restructure (below). |
...
| 13 | §2.1 | are AloePri/STIP the only static-obf schemes? | ⏳ | Need a caption/line noting the table is *representative* (full set in §5 `tab:obfusc`). |
...
| 15 | §2.3 | "of section 1" — too much retroactive referencing | 🟡 | Dropped this + a couple; broad retroactive-ref reduction still pending. |
| 16 | §2.3 | vignette constrained to AloePri — one-sided | ⏳ | Decided: move to appendix, reframe to heuristic-basis deployment-ready set (AloePri/GELO/ObfuscaTune/SGT/OSNIP), finding + reproduction protocol. **Not yet built.** |
...
| 19 | §2.2 | head-to-head / self-reported / provenance unclear | ✅ | Restructured to "indicative by default / co-measured"; swept §2+§8 (1 residual inside the §5 vignette block, pending its restructure). |
...
| 23 | §3.1 | "(subsection 2.2)" retroactive ref | 🟡 | Now `§2.2`; broad reduction pending. |
| 24 | §4 | TEE table appears before its reference — anchor after | ⏳ | General table-placement pass (the crypto Table 5/6 drift was fixed with `[H]`; §4 TEE table still to do). |
...
| 30 | §5 | vignette surface/companion paragraph — verbose/redundant | ⏳ | Trim, part of the vignette restructure. |
